    ...
    IMessageController
{
    ...
    modifier onlyMessenger(uint32 spokeNetwork) {
        require(
            msg.sender == address(messengers[spokeNetwork]),
            "unauthorized"
        );
        _;
    }
    ...
    function finishCollectionPhase(uint256 proposalId) public {
        ...
        for (uint16 i = 0; i < spokeNetworks.length && phaseFinished; i++) {
            phaseFinished =
                phaseFinished &&
                spokeVotes[proposalId][spokeNetworks[i]].initialized;
        }
    ...
    function requestCollections(
        ...
        require(
            ...
            "the collection phase has already started"
        );
        for (uint16 i = 0; i < spokeNetworks.length; i++) {
            messengers[spokeNetworks[i]].requestCollection(
                proposalId,
    ...
    function onReceiveSpokeVotingData(
        ...
        require(
            !spokeVotes[proposalId][spokeNetwork].initialized,
            "Aready initialized!"
        );
        spokeVotes[proposalId][spokeNetwork] = SpokeProposalVote(
            forVotes,
            againstVotes,
            abstainVotes,
            true
