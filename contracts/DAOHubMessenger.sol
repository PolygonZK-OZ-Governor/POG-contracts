...
contract DAOHubMessenger is PolygonBridgeBase {
    ...
    function requestCollection(uint256 proposalId, bool forceUpdateGlobalExitRoot) external {
        // Encode message data
        bytes memory messageData = abi.encode(REQUEST_COLLECTION_SIG, abi.encode(proposalId));
    ...
    function bridgeProposal(
        ...
        bytes memory messageData = abi.encode(
            BRIDGE_PROPOSAL_SIG,
            abi.encode(proposalId, proposalStart, snapshot, deadline)
        );
    ...
    function _onReceiveSpokeVotingData(bytes memory payload) internal {
        (uint256 proposalId, uint256 forVotes, uint256 againstVotes, uint256 abstainVotes) = abi.decode(
            payload,
