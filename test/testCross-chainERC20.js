...
describe("Deploy", () => {
    ...
    it(" tranfer token from network 1 to network 0 ", async () => {
        ...
        expect(
            await testnetPolygonZkEVMBridgeContract.verifyMerkleProof(leafValue, proof, index, rootSCMainnet)
        ).to.be.equal(true);
        ...
        computedGlobalExitRoot = calculateGlobalExitRoot(
    ...
    it(" tranfer token from network 0 to network 1 ", async () => {
        ...
        console.log(
            `balance of user[1] before tranfer at mainNet = `,
            Number(await tokenHub.balanceOf(user[1].address))
        );
        //console.log(`balance of bridge at mainNet = `, await tokenHub.balanceOf(tokenBrigdeContractMainnet.address));
        console.log(
            `balance of user[2] before tranfer at testNet = `,
            Number(await tokenSatellite.balanceOf(user[2].address))
        );
        ...
        expect(verifyMerkleProof(leafValue, proof, index, rootSCMainnet)).to.be.equal(true);
        expect(
            await mainnetPolygonZkEVMBridgeContract.verifyMerkleProof(leafValue, proof, index, rootSCMainnet)
        ).to.be.equal(true);
        let computedGlobalExitRoot = calculateGlobalExitRoot(rootJSMainnet, rollupExitRoot);
        expect(computedGlobalExitRoot).to.be.equal(await mainnetPolygonZkEVMGlobalExitRoot.getLastGlobalExitRoot());
